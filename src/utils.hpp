...
#ifdef VERBOSE
#define VERBOSE_DEBUG(msg) {Rcpp::Rcout  << msg << std::endl;};
#else
#define VERBOSE_DEBUG(msg) {};
#endif
#define VERBOSE_INFO(msg)  {Rcpp::Rcerr  << msg << std::endl;};
...
// TODO : We took that from Rcpp-dist, our code must be GPL !
...
inline double fast_sample_raf(const arma::vec& weights ) { // sample_raf when (plus1 == 0) and (hm == 1)
	const double somma = sum(weights); // TODO : Must be 1 not ??
	...
	// TODO What happen if condition not met ?
